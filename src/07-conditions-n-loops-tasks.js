...
function isInsideCircle(circle, point) {
  const value = Math.sqrt((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2);
...
function isCreditCardNumber(ccn) {
  const nums = ccn.toString().split('').map((num) => +num);
  let sum = 0;
  let index = 0;
  if ((nums.length - 1) % 2 === 0) {
    index = 1;
  }
  for (index; index < nums.length; index += 2) {
    let current = 2 * nums[index];
    if (current > 9) {
      current -= 9;
    }
    nums[index] = current;
  }
  sum = nums.reduce((acc, el) => acc + el);
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  const nums = num.toString().split('').map((n) => +n);
